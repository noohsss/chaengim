...
- 웹 알림과 읽음 상태는 `notifications` 한 행에 저장한다.
- 이메일 관련 컬럼은 선행 마이그레이션에 남아 있지만 이번 MVP에서는 사용하지 않는다. 이메일 알림은 후속 범위에서 별도 정리한다.
...
OAuth 로그인 사용자의 최소 프로필이다. 기존 마이그레이션의 이메일 관련 컬럼은 이메일 알림 후속 작업을 위한 호환 필드로 남아 있으며 현재 런타임에서는 사용하지 않는다.
...
웹 알림과 읽음 상태를 관리한다. 기존 마이그레이션의 이메일 발송 상태 컬럼은 후속 이메일 알림 작업 전까지 사용하지 않는다.
...
- 정책 병합 이력
- 신청 결과
- 이메일 발송 이력(이메일 알림 후속 범위)
- 동기화 실행 이력
- 정책 변경 이력

이 데이터는 기존 5개 테이블의 배열·JSON 컬럼 또는 Vercel·Supabase 로그로 처리한다.
